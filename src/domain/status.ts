...
/**
 * A reason (or an `origin`) and the data behind it, as one phrase — `BuyGuard ·
 * upperLimit: 119.34`, `Retry · count: 0`. The data hangs off the key on the
 * same middle dot the qualifier line uses elsewhere, and each pair is joined by
 * a colon; several pairs are separated by commas, the one separator the dots
 * are not already spending. Keys and values both print **exactly as the server
 * sent them** — the number is not run through the page's Turkish figure form,
 * because `reasonData` / `originData` carry raw server fields (a `count`, a
 * default's name like `ceilingAtClosingDay`), not figures the page owns. A
 * value of another shape is left out rather than guessed at, and a key with no
 * data is just itself.
 */
export function reasonPhrase(reason: string, data: ReasonData | null): string {
  ...
  return pairs.length === 0 ? reason : `${reason} · ${pairs.join(', ')}`;
}
